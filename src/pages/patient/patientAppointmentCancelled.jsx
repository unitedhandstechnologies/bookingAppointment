                  ...
                  <div className="row my-4">
                    <div className="search-bar-text-input col-md-7 top-search">
                      ...
                      <div className="col-lg-4 float-start">
                        <input type="text"
                          className="form-control"
                          placeholder={t('Patient.AppointmentCancelled.Search_Appointment_Doctor_Name')}
                    ...
                    <div className="search-bar-text-input col-md-2">
                      <button type="button" onClick={this.GetAppointmentRequestList.bind(this)} className="btn filter-btn w-100">{t('Patient.AppointmentCancelled.Search')}</button>
                    </div>
                  </div>
                        ...
                        <table className="table table-bordered appointmentTable">
                          <thead>
                            <tr className="new-patient-table-title">
                              <th rowspan="2">{t('Patient.AppointmentCancelled.Booking_ID')}</th>
                              <th rowspan="2">{t('Patient.AppointmentCancelled.Doctor_Name')}</th>
                              <th>{t('Patient.AppointmentCancelled.Date')}</th>
                              <th rowspan="2">{t('Patient.AppointmentCancelled.Medical_Specialty')}</th>
                              <th rowspan="2">{t('Patient.AppointmentCancelled.Amount_(€)')}</th>
                              <th rowspan="2">{t('Patient.AppointmentCancelled.Appointment_Type')}</th>
                              <th rowspan="2">{t('Patient.AppointmentCancelled.Cancelled_By')}</th>
                              <th rowspan="2">{t('Patient.AppointmentCancelled.Reason')}</th>
                            ...
                            <tbody>
                              {this.state.AppointmentRequestList.length > 0 ? this.state.AppointmentRequestList.map((v, idx) => (
                                <tr>
                                  ...
                                  <td className="dateTime">{moment(v.appointmentDateTime).format("DD/MM/YYYY")}<br />{moment(v.appointmentDateTime).format("HH:mm")}</td>
                                  ...
                                  <td className="type">{v.cancelledByStr}</td>
                                  <td className="type-option-canceled reason">{v.cancelReason}</td>
                                  <td className="type">{moment(v.suggestedDateTime).format("DD/MM/YYYY")} {moment(v.suggestedDateTime).format("HH:mm")}</td>
                                </tr>
                              )) : <tr><td colSpan={9} className="empty-list">{t("EmptyListMessages.cancelled_appointments")}</td></tr>
                              }
                            </tbody>
                          }
                        </table>
...
export default connect(mapStoreToprops, mapDispatchToProps)(withTranslation()(PatientAppointmentCancelled));
          ...
          setCommissionList(
            data.data.result.map((list) => {
              return {
                ...
                commissionPercentage: list.commissionPercentage
                  ? list.commissionPercentage.toFixed(2)
                  : (0).toFixed(2),
  ...
  const handleSubmit = () => {
    const data = commissionList.map((list) => {
      return {
        ...
        doctorGuid: list.doctorGuid,
        commissionPercentage: list.commissionPercentage
          ? parseFloat(list.commissionPercentage)
          : 0,
        firstName: list.firstName,
        lastName: list.lastName,
      };
    });
    promiseWrapper(props.adminactions.saveDoctorCommission, {
              ...
              <div className="col-md-12 col-sm-12 col-lg-10 mainRightPanel">
                ...
                >
                  <div className="col-md-12">
                      ...
                      <table className="table table-bordered appointmentTable">
                        ...
                        <tbody>
                          {commissionList.length ? (
                            commissionList.map((data, i) => (
                              <tr key={i}>
                                <td>
                                  <Link
                                    className="doctor-name"
                                    to={`/doctor-detail-view/${data.doctorGuid}`}
                                  >
                            ...
                            <tr>
                              <td colSpan={2} className="empty-list">
                                {t("EmptyListMessages.commissions_list")}
                              </td>
                            </tr>
                          )}
                        </tbody>
                    ...
                    <button
                      type="button"
                      className="btn btn-success float-end mt-5 me-5"

...
class CmsHome extends Component {
   ...
   componentDidMount() {
      this.getCMSPageData(cmsPageNum.HomePage, this.props.langId ? this.props.langId : 1, this.state.isAdmin)
   }
   ...
   componentDidUpdate() {
      if (this.props.langId !== this.state.langId) {
         this.setState({ langId: this.props.langId, cmsData: { ...this.state.cmsData, languageId: this.props.langId } }, () => {
            this.getCMSPageData(cmsPageNum.HomePage, this.props.langId, this.state.isAdmin)
         })
      }
   }
   ...
   render() {
      ...
      const {
         headerMainTitle, headerSubTitle, offlineConsultation,
         onlineConsultation, chatWithADoctor, midSectionTitle,
         howItWorksStep1, howItWorksStep2, howItWorksStep3,
         howItWorksStep4, howItWorksStep5, emergencyOnlineConsultation,
         describeInShort, talkwithDoctor
      } = this.state.cmsData;
      ...
      return (
         <div className='appointment-details-container'>
            <form className="appointment-details-form" onSubmit={(e) => { e.preventDefault(); this.submitCMSData() }}>
               ...
               <div className="form-group my-4">
                  <label for="describe-problem-textarea">Emergency On-line Consultation</label>
                  <textarea
                     className="form-control appointment-details-form-inputs"
                     onChange={(e) => this.setStates(e)}
                     rows="3"
                     name='emergencyOnlineConsultation'
                     value={emergencyOnlineConsultation ? emergencyOnlineConsultation : ""}
                  ></textarea>
               </div>
               <div className="form-group my-4">

...
export async function fetchNextAppointments(appointmentModel, doctorGuid, isViewAllAvailability) {
...
export async function cancelAppointment(appointmentGuid, cancelReason, canceledBy, suggestedDateTime) {
  const config = getConfig('exadoPatient.CancelAppointment');
  config.urlParams = { appointmentGuid, cancelReason, canceledBy, suggestedDateTime };
  const apiInstance = new ApiService(config);
  return apiInstance.call();
}

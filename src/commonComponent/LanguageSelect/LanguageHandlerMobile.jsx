...
class LanguageHandlerMobile extends React.Component {
  ...
  componentDidMount() {
    let cl =
      cookie.load("current-language-name") === undefined
        ? "English"
        : cookie.load("current-language-name");
    cookie.save("current-language-name", cl, { path: "/" });
    this.getWebsiteLanguages(cl);
  }
  componentDidUpdate() {
    if (this.props.languageObj) {
      const { languageAbbreviation, languageName, languageId } =
        this.props.languageObj;
  ...
  changeLanguage = (lan, lanName) => {
    ...
    const langObj = this.getLanguageObject(
      this.state.WebSiteLanguageData,
      lanName
    );
    this.changeStateLanguage(langObj);
  };
  ...
  render() {
    return (
      <div
        className="modal fade mobile-nav-modal-language"
        id="mobilemodallanguage"
        data-backdrop="static"
        data-keyboard="false"
        tabIndex="-1"
        aria-labelledby="mobilemodallanguageLabel"
        aria-hidden="true"
      >
...
export default connect(
  mapStoreToprops,
  mapDispatchToProps
)(withTranslation()(LanguageHandlerMobile));

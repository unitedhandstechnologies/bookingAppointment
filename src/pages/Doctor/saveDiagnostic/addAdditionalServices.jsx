...
export class AddAdditionalServices extends Component {
   ...
   getDoctorAdditionalService = () => {
      promiseWrapper(this.props.docactions.getAllDoctorAdditionalService, { params: { doctorGuid: localStorage.getItem("user-id") } })
         .then(data => this.setState({ additionalServicesList: data.data, selectedServiceGuid: data.data[0].serviceGuid, options: data.data }, () => this.getAdditionalServiceInfo()))
         .catch(err => console.log(err))
   }

   getAdditionalServiceInfo = () => {
      promiseWrapper(this.props.docactions.getAdditionalServiceInfo, { params: { appointmentGuid: this.props.appointmentGuid } })
         .then(data => {
      ...
      if (array) {
         for (const serviceAdd of array) {
            ...
            for (const service of this.state.options) {
               if (service.serviceGuid === serviceAdd.serviceGuid) service.isAdded = true
            };
         }
      }
      else {
         ...
         for (const optionService of this.state.options) {
            if (service.serviceGuid === optionService.serviceGuid) optionService.isAdded = true
         };
      }
      const additionalAmount = parseFloat(this.totalAdditionalAmount(this.state.addedServices));
      this.props.addAdditionalServiceData(this.state.addedServices, additionalAmount)
      this.setState({ addedServices: this.state.addedServices, options: this.state.options })
   }
   ...
   removeAddedService = (service) => {
      ...
      for (const addedService of this.state.options) {
         if (addedService.serviceGuid === service.serviceGuid) addedService.isAdded = false
      }
      const additionalAmount = parseFloat(this.totalAdditionalAmount(this.state.addedServices));
      this.props.addAdditionalServiceData(this.state.addedServices, additionalAmount)
      this.setState({ addedServices: this.state.addedServices, options: this.state.options })
         ...
         <>
            <div className="input-group mt-4">
               <input
                  ...
                  placeholder={t("Doctor.AddAdditionalServices.Search_Additional_Services")}
                  onChange={(e) => this.onSearchInputChange(e)}
                  onFocus={() => this.setState({ dropDownOpen: true })}
               />
               {dropDownOpen ?
                  <button className="input-group-text" type='button' onClick={() => this.setState({ dropDownOpen: false })}><i className="fas fa-sort-down"></i></button> :
                  <button className="input-group-text" type='button' onClick={() => this.setState({ dropDownOpen: true })}><i className="fas fa-caret-right"></i></button>
               }
            </div>
            {dropDownOpen &&
                  ...
                  <table className="table table-bordered appointmentTable">
                     <thead>
                        <tr className="new-patient-table-title">
                           <th className='text-center'>{t("Doctor.AddAdditionalServices.Service_Name")}</th>
                           <th className='text-center'>{t("Doctor.AddAdditionalServices.Description")}</th>
                           <th className='text-center'>{t("Doctor.AddAdditionalServices.Amount")}</th>
                           <th className='text-center'>{t("Doctor.AddAdditionalServices.Action")}</th>
                        </tr>
                     </thead>
                     <tbody>

...
const AdminUnprocessedWithdrawList = (props) => {
    ...
    const getPageData = () => {
        promiseWrapper(
            props.adminactions.getAllUnprocessedWithdraw, {
            paging: {
                "pageSize": 10,
                                    ...
                                    <div className="col-md-12">
                                        ...
                                        <div className="tableContainer table-responsive">
                                            <table className="table table-bordered appointmentTable">
                                                ...
                                                <tbody>
                                                    {withdrawList.length > 0 ? withdrawList.map((data, i) => (
                                                        <tr key={i}>
                                                            <td>{i + 1}</td>
                                                            <td>
                                                                <Link className="patientName" to={`/patient-detail-view/${data.patientGuid}`}
                                                                >
                                                                    {data.patientFirstName} {data.patientLastName}
                                                                </Link>
                                                            </td>
                                                            <td className="profileVerificationStatus">€ {data.withdrawAmount}</td>
                                                    ...
                                                    )) : <tr><td colSpan={4} className="empty-list"> Withdrawal requests are not available.</td></tr>}
                                                </tbody>
                                            </table>
...
export default connect(mapStoreToprops, mapDispatchToProps)(AdminUnprocessedWithdrawList);
...
class CmsPatientQuestions extends Component {
  ...
  getCMSPatientQuestions = () => {
    promiseWrapper(this.props.adminactions.getPatientQuestionnaireLanguage, {
  ...
  submitCMSData = () => {
    if (this.state.langId === 1) {
      ...
      if (!isEmpty)
        promiseWrapper(
          this.props.adminactions.savePatientQuestionnaireLanguage,
      ...
      promiseWrapper(this.props.adminactions.savePatientQuestionnaireLanguage, {
        data: { questionnaireData: this.state.cmsData },
      })
        .then((data) => {
          if (data.data.isSuccess) toast.success(data.data.message);

...
class CmsEmergency extends Component {
   ...
   componentDidMount() {
      this.getCMSPageData(cmsPageNum.Emergency, this.props.langId ? this.props.langId : 1, this.state.isAdmin)
   }
   ...
   componentDidUpdate() {
      if (this.props.langId !== this.state.langId) {
         this.setState({ langId: this.props.langId, cmsData: { ...this.state.cmsData, languageId: this.props.langId } }, () => {
            this.getCMSPageData(cmsPageNum.Emergency, this.props.langId, this.state.isAdmin)
         })
      }
   }

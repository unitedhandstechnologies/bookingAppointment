            ...
            <table className="table table-bordered appointmentTable">
              <thead>
                <tr className="new-patient-table-title">
                  <th rowspan="2">
                    {t("Admin.DoctorDetailView.Appointments.Booking_ID")}
                  </th>
                  <th rowspan="2">
                    {t("Admin.DoctorDetailView.Appointments.Patient_Name")}
                  </th>
                  <th>{t("Date")}</th>
                  <th rowspan="2">
                    {t("Admin.DoctorDetailView.Appointments.Amount")} (€)
                  </th>
                  <th rowspan="2">
                    {t("Admin.DoctorDetailView.Appointments.Appointment_Type")}
                  </th>
                  <th rowspan="2">
                    {t("Admin.DoctorDetailView.Appointments.Refund_Amount")}
                  </th>
                  <th rowspan="2">
                    {t("Admin.DoctorDetailView.Appointments.Admin_Commission")}
                  </th>
                  <th rowspan="2">
                    {t("Admin.DoctorDetailView.Appointments.Due_Amount")}
                  </th>
                </tr>
                <tr>
                  ...
                  {this.state.AppointmentList.map((data, i) => (
                    <tr key={i}>
                      ...
                      <td className="dateTime">
                        {moment(data.appointmentDateTime).format("MM/DD/YYYY")}
                        <br />
                        {moment(data.appointmentDateTime).format("HH:mm")}
                      </td>

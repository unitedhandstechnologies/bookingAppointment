...
class AppointmentCancel extends React.Component {
  ...
  CancelAppointment() {
    const dateNtime =
      this.state.appointmentDate && this.state.appointmentTime
        ? `${this.state.appointmentDate}T${this.state.appointmentTime}`
        : new Date().toUTCString();
    if (this.state.cancelReason !== "" && this.state.cancelReason !== null) {
      promiseWrapper(this.props.patientactions.cancelAppointment, {
        appointmentGuid: this.props.AppointmentGuid,
        cancelReason: this.state.cancelReason,
        canceledBy: localStorage.getItem("user-type"),
        suggestedDateTime: dateNtime,
      }).then((data) => {
  ...
  render() {
    const { t } = this.props;
    const { availableSlotes, appointmentDate } = this.state;
    return (
      <div
      ...
      >
        <div className="modal-dialog" role="document">
          <div className="modal-content">
            <a
              type="button"
              className="close modify-modal-close text-end mt-3 me-3"
              ...
              <div className="divForm my-4">
                <div className="d-flex flex-column text-center">
                  <div className="patient-forget-password-text">
                    <p>
                      {t(
                        "Doctor.AppointmentCancelledPopup.Appointment_Cancelled"
                      )}
                    </p>
                  </div>
                </div>
                ...
                <label className="mb-2">
                  {t(
                    "Doctor.AppointmentCancelledPopup.Please_enter_your_reason"
                  )}
                </label>
                <div className="search-bar-text-input delete-account-textarea mb-3">
                  <textarea
                    className="form-control"
                    onChange={this.UpdateCancelreason.bind(this)}
                    rows="4"
                    placeholder={t(
                      "Doctor.AppointmentCancelledPopup.Your_Reason"
                    )}
                  ></textarea>
                </div>
                <div className="mt-3 text-center">
                  ...
                  >
                    {t("Doctor.AppointmentCancelledPopup.Send_Cancelation")}
                  </a>
                </div>
              </div>
            </div>
...
export default connect(
  mapStoreToprops,
  mapDispatchToProps
)(withTranslation()(AppointmentCancel));

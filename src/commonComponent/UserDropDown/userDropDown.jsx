...
class UserDropDown extends React.Component {
    ...
    LogoutUser = (event) => {
        ...
        localStorage.clear();
        if (this.props.history.location.pathname === "/" && userType !== "3") this.props.logOutAction();
                                ...
                                <div className="profilePhotoContainer">
                                    <div className="profilePhoto d-flex justify-content-center align-items-center">
                                        <img
                                            ...
                                            style={{ borderRadius: "inherit" }} className="img-fluid" alt='profile'
                                        />
                                    </div>
                                </div>
                                        ...
                                        <div className="row">
                                            {localStorage.getItem("user-type") == "3" &&
                                                <div className="col-md-12">
                                                    <Link className="dropdown-item" to={`/admin-dashboard`}>{t("Doctor.DocHeader.Overview")}</Link>
                                                    <div className="dropdown-item" onClick={this.LogoutUser.bind(this)}>{t("Doctor.DocHeader.Logout")}</div>
                                                </div>
                                            }
                                            {localStorage.getItem("user-type") == "2" &&
                                                <div className="col-md-12">
                                                    <Link className="dropdown-item" to={`/patient-dashboard`}>{t("Doctor.DocHeader.Overview")}</Link>
                                                    <Link className="dropdown-item" to={`/patient-profile`}>{t("Doctor.DocHeader.My_Profile")}</Link>
                                                    <div className="dropdown-item" onClick={this.LogoutUser.bind(this)}>{t("Doctor.DocHeader.Logout")}</div>
                                                </div>
                                            }
                                            {localStorage.getItem("user-type") == "1" &&
                                                <div className="col-md-12">
                                                    <Link className="dropdown-item" to={`/doctor-dashboard`}>{t("Doctor.DocHeader.Overview")}</Link>
                                                    <Link className="dropdown-item" to={`/doctor-profile`}>{t("Doctor.DocHeader.My_Profile")}</Link>
                                                    <div className="dropdown-item" onClick={this.LogoutUser.bind(this)}>{t("Doctor.DocHeader.Logout")}</div>
                                                </div>
                                            }
                                        </div>

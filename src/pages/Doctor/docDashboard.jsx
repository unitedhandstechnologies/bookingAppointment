...
import { isAppointmentNew, localStorageKeys, promiseWrapper } from '../../utility/common';
...
class DoctorDashboard extends React.Component {
  ...
  componentWillUnmount() { clearTimeout(this.state.timer) }
  ...
  GetAppointmentRequestList() {
    ...
    promiseWrapper(this.props.patientactions.getAppointments, { filter: param }).then((data) => {
      const requestList = data.patientAppointments.filter(appointmentData => (appointmentData.appointmentStatus === "Requested" && isAppointmentNew(appointmentData.appointmentDateTime)));
      const upcomingList = data.patientAppointments.filter(appointmentData => appointmentData.appointmentStatus === "Approved" && isAppointmentNew(appointmentData.appointmentDateTime, 2))
      this.setState({
  ...
  getUserLanguage = () => {
    promiseWrapper(this.props.docactions.getProfileInfo, { userGuid: localStorage.getItem(localStorageKeys.userId) }).then(data => {
      if (data.languageId) {
        const languageData = JSON.parse(localStorage.getItem(localStorageKeys.websiteLanguageData))
        const userLangugae = languageData.find(language => language.languageId === data.languageId)
        if (userLangugae?.languageId) {
          this.setState({ timer: setTimeout(() => promiseWrapper(this.props.comactions.changeLanguage, { language: userLangugae }), 500) })
        }
      }
    })
  }
            ...
            <div className="row">
              ...
              <div className="col-md-12 col-sm-12 col-lg-10 mainRightPanel">
                <div className="row">
                  <div className="col-md-3 col-6 my-3">
                    <div className="card doctor-card">
                      <div className="doctor-card-title pt-3">{t('Doctor.MyDashboard.Total_Earning')}</div>
                      <div className="doctor-card-text pb-3">{t('Doctor.MyDashboard.€_1500')}</div>
                    </div>
                  </div>
                  <div className="col-md-3 col-6 my-3">
                    <div className="card doctor-card">
                      <div className="doctor-card-title pt-3">{t('Doctor.MyDashboard.Total_Appointment')}</div>
                      <div className="doctor-card-text pb-3">{t('Doctor.MyDashboard.M_1560')}</div>
                    </div>
                  </div>
                  <div className="col-md-3 col-6 my-3">
                    <div className="card doctor-card">
                      <div className="doctor-card-title pt-3">{t('Doctor.MyDashboard.Total_Patients')}</div>
                      <div className="doctor-card-text pb-3">{t('Doctor.MyDashboard.M_163')}</div>
                    </div>
                  </div>
                  <div className="col-md-3 col-6 my-3">
                    <div className="card doctor-card">
                      <div className="doctor-card-title pt-3">{t('Doctor.MyDashboard.Total_Consultancy_Hours')}</div>
                      <div className="doctor-card-text pb-3">{t('Doctor.MyDashboard.M_102')}</div>
                    </div>
                  </div>
                </div>
                <div className="row d-flex justify-content-center pt-5 bg-white">
                    ...
                    {this.state && this.state.LoadedData &&
                      <div className="card doctor-table-card mb-3">
                        <div className="doctor-table-card-title mb-3 py-4 px-3 d-flex justify-content-between">
                          <div>{t('Doctor.MyDashboard.Appointment_Requests')}</div>
                          <div className="doctor-table-card-title-link">
                            <Link className="text-light" to="/doctor-appointment-requests">{t('Doctor.MyDashboard.View_All')}</Link>
                          </div>
                        </div>
                        {appointmentRequestList.length ?
                          appointmentRequestList.map((v, idx) =>
                            <div className="doctor-table-card-body mx-3" key={idx}>
                              <div className="row mb-2">
                                <div className="col-md-4">
                                  <div className="mb-2">
                                    <span className="doctorName">
                                      <Link className="doctorName" to={"/patient-detail-view/" + v.patientGuid}>{v.patientFirstName} {v.patientLastName}</Link></span>
                                    |<span className="medicalSpecialty"> {v.appointmentType}</span>
                                  </div>
                                  <div>{v.physicianServices}</div>
                                </div>
                                <div className="col-md-8 doctor-table-card-body-right">
                                  <div>
                                    <div className="dateTime mb-2 w-100">{moment(v.appointmentDateTime).format("MM/DD/YYYY")} {moment(v.appointmentDateTime).format("HH:mm")}</div>
                                    <div className="action">
                                      <div className="d-flex justify-content-end">
                                        <a className="btn viewDetails doctor-viewDetails" onClick={this.toggleViewAppointment.bind(this, v.appointmentGuid)} role="button">{t('Doctor.MyDashboard.View_Details')}</a>
                                        <a className="btn joinCall mr-2" onClick={this.IsAcceeptAppoinment.bind(this, v.appointmentGuid)} role="button">{t('Doctor.MyDashboard.Accept')}</a>
                                        <button type="button" onClick={this.IsRejectAppoinment.bind(this, v.appointmentGuid)} className="btn btn-outline-dark cancel">{t('Doctor.MyDashboard.Reject')}</button>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                          ...
                          ) : <div className="text-center px-2 empty-list">{t("EmptyListMessages.appointments_request")}</div>
                        }
                      </div>
                    }
                            ...
                            <div className="modal-body">
                              <div className="divForm my-4">
                                ...
                                <div className="mt-3 text-center">
                                  <a className="btn MyButton reset-password-button w-75 py-2" onClick={this.MarkAppointmentAsCompleteWithDiagnostic.bind(this, "")}>{t('Doctor.MyDashboard.Yes,_Upload_diagnostic_report')}</a>
                                </div>
                                <div className="mt-3 text-center">
                                  <a className="btn MyButton reset-password-button w-75 py-2" onClick={this.MarkAppointmentAsComplete.bind(this, "")}>{t('Doctor.MyDashboard.Yes,_dont_Upload_documents')}</a>
                                </div>
                                <div className="mt-3 text-center">
                  ...
                  {this.state.IsCancelPopUp ? <AppointmentCancelPopup ClosePopup={this.CloseCancelPopup.bind(this)} AppointmentGuid={this.state.appointmentGuid} /> : null}
                  <div className="col-lg-6 col-md-12">
                    {this.state && this.state.LoadedData &&
                      <div className="card doctor-table-card mb-3">
                        <div className="doctor-table-card-title mb-3 py-4 px-3 d-flex justify-content-between">
                          <div>{t('Doctor.MyDashboard.Upcoming_Appointment')}</div>
                          <div className="doctor-table-card-title-link"><Link className="text-light" to="/doctor-appointment-upcoming">{t('Doctor.MyDashboard.View_All')}</Link></div>
                        </div>
                        {appointmentUpcomingList.length > 0 ? appointmentUpcomingList.map((v, idx) => (
                          <div className="doctor-table-card-body mx-3">
                            <div className="row mb-2">
                              <div className="col-md-4">
                                <div className="mb-2">
                                  <Link className="doctorName" to={"/patient-detail-view/" + v.patientGuid}>{v.patientFirstName} {v.patientLastName}</Link>
                                  |<span className="medicalSpecialty"> {v.appointmentType}</span>
                                </div>
                                <div>{v.physicianServices}</div>
                              </div>
                                    ...
                                    <div className="d-flex justify-content-end">
                                      ...
                                      <div className="dropdown">
                                        ...
                                        <div className="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                          ...
                                          <a className="dropdown-item dropdown-action" onClick={this.MakeCompletePopUpClick.bind(this, v.appointmentGuid)}>{t('Doctor.MyDashboard.Mark_as_complete')}</a>
                                          <a className="dropdown-item dropdown-action" onClick={this.IsCancelAppoinment.bind(this, v.appointmentGuid)}>{t('Doctor.MyDashboard.Cancel')}</a>
                                        </div>
                                      </div>
                        ...
                        )) : <div className="text-center px-2 empty-list">{t("EmptyListMessages.upcoming_appointments")}</div>}
                      </div>
                    }
                  </div>
                </div>
...
export default connect(mapStoreToprops, mapDispatchToProps)(withTranslation()(DoctorDashboard));
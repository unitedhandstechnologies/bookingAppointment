...
class CmsApiMessages extends Component {
  ...
  getCMSMessages = () => {
    promiseWrapper(this.props.adminactions.getLanguageAllMessages, {
  ...
  submitCMSData = () => {
    ...
    if (this.state.langId === 1) {
      this.state.cmsData.forEach((input) => {
        if (!input.languageMessage) isEmpty = true;
      });
      if (!isEmpty)
        promiseWrapper(this.props.adminactions.saveLanguageLabelData, {

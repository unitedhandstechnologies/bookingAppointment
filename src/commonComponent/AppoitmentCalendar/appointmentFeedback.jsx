...
class AppointmentFeedBack extends React.Component {
    ...
    SubmitFeedBack() {
        ...
        if (this.state.FeedBackComments !== "" && this.state.FeedBackComments !== null) {
            promiseWrapper(this.props.patientactions.saveReview, { model: data }).then((data) => {
                    ...
                    <div className="modal-content">
                        <div className="modal-body">
                            <div className="divForm my-4">
                                ...
                                <label className="mb-2">{t('Doctor.PatientFeedback.Provide_your_feedback')}</label>
                                <div className="search-bar-text-input delete-account-textarea">
                                    <textarea className="form-control" onChange={this.UpdateFeedBackComments.bind(this)} rows="4" placeholder={t('Doctor.PatientFeedback.Your_Feedback')}></textarea>
                                </div>
                                <div className="mt-2 mb-5 d-flex align-items-center">
                                    <span className="feedback-star-rating-text">{t('Doctor.PatientFeedback.Star_Rating')}</span>
                                    <ReactStars
                                ...
                                <div className="mt-3 text-center">
                                    <button type="button" className="btn pending-feedback-cancel w-25 mx-2" onClick={this.ClosePopupClick.bind(this)}>{t('Doctor.PatientFeedback.Cancel')}</button>
                                    <a className="btn MyButton reset-password-button w-25 mx-2" onClick={this.SubmitFeedBack.bind(this)} >{t('Doctor.PatientFeedback.Done')}</a>
                                </div>
                            </div>
                        </div>
                    </div>
...
export default connect(mapStoreToprops, mapDispatchToProps)(withTranslation()(AppointmentFeedBack));
...
class CommonCalender extends React.Component {
    ...
    componentDidUpdate() {
        if (this.state.anAppoinmentModel.appointmentType !== this.props.appoinmentModel.appointmentType) {
    ...
    CalendarClick(mov) {
        let appdate = "";
        if (mov === 1) {
            appdate = moment(this.state.CalanderDisplayData[0].currentDate).subtract(5, "days").format("YYYY-MM-DD");
        }
        else {
            appdate = moment(this.state.CalanderDisplayData[4].currentDate).add(1, "days").format("YYYY-MM-DD");
        }
    ...
    CallFetchNextAppointments = () => {
        promiseWrapper(this.props.patientactions.fetchNextAppointments, { appointmentModel: this.state.anAppoinmentModel, doctorGuid: this.props.DocGuid, isViewAllAvailability: false }).then((jsdata) => {
                                ...
                                <div className="modal-body">
                                    <div className="divForm my-4">
                                        ...
                                        <div className="text-center mb-5">
                                            <span>You Need To Login Before Booking An Appointment</span><br />
                                        </div>
                                        <div className="mt-3 text-center">
                                            <Link className="btn MyButton reset-password-button w-50" to="/login">Login</Link>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div className="row">
                                    <div className="col-md-1 col-1 text-center d-flex justify-content-end">
                                        {this.state.CalanderDisplayData.length > 0 &&
                                            <button className='calender-icon'
                                                disabled={new Date(this.state.CalanderDisplayData[0].currentDate).getTime() < new Date().getTime() + 24 * 60 * 60 * 1000}
                                                onClick={this.CalendarClick.bind(this, 1)}><i className="fas fa-chevron-left"></i>
                                            </button>
                                        }
                                        <br />
                                    </div>
                                    {this.state.CalanderDisplayData.map((v, idx) => (
                                        <div className="col-md-2 col-2 text-center" key={idx}>
                                            <div className="date-align">
                                                <span>{getMonthNameByDate(new Date(v.currentDate))} {new Date(v.currentDate).getDate()}</span><br />
                                                <span>{getDayNameByDate(new Date(v.currentDate)).sortName}</span>
                                            </div>
                                            {v.availableTime.map((d, idx) => (
                                                <div className="timing-btn" key={idx}>
                                                    {d !== "0001-01-01T00:00:00" &&
                                                        <button className='calender-timing-btn'
                                    ...
                                    <div className="col-md-1 col-1 text-center d-flex justify-content-end">
                                        <button className='calender-icon' onClick={this.CalendarClick.bind(this, 2)}><i className="fas fa-chevron-right"></i></button><br />
                                    </div>
                                </div>
                            </div>
                        </div>

...
class PatientAppointmentCompleted extends React.Component {
  ...
  ShowRefundPopUp = (data) => {
    this.setState({ RefundAppointmentData: this.state.AppointmentRequestList.filter(d => d.appointmentGuid === data)[0] }, () => {
  ...
  FeedbackViewPopUp = (data) => {
    if (data !== "") {
      ...
      promiseWrapper(this.props.patientactions.getDoctorReviewByAppointmentId, { appointmentGuid: data }).then((data) => {
  ...
  UpdateShowDiagnosisToOthers = (checked, event, id) => {
    promiseWrapper(this.props.patientactions.showDiagnosisToOthers, { appointmentGuid: id, bValue: checked })
      .then((data) => {
      ...
      <div>
        ...
        <div className="main">
          <div className="container-fluid">
                  ...
                  <div className="row my-4">
                    <div className="search-bar-text-input col-md-7 top-search">
                      ...
                      <div className="col-lg-4 float-start">
                        <input type="text"
                          className="form-control"
                          placeholder={t('Patient.AppointmentCompleted.Search_Appointment_Doctor_Name')}
                    ...
                    <div className="search-bar-text-input col-md-2">
                      <button type="button" onClick={this.GetAppointmentRequestList.bind(this)} className="btn filter-btn w-100">{t('Patient.AppointmentCompleted.Search')}</button>
                    </div>
                  </div>
                        ...
                        <table className="table table-bordered appointmentTable">
                          <thead>
                            <tr className="new-patient-table-title">
                              <th rowspan="2">{t('Patient.AppointmentCompleted.Booking_ID')}</th>
                              <th rowspan="2">{t('Patient.AppointmentCompleted.Doctor_Name')}</th>
                              <th>{t('Patient.AppointmentCompleted.Date')}</th>
                              <th rowspan="2">{t('Patient.AppointmentCompleted.Medical_Specialty')}</th>
                              <th rowspan="2">{t('Patient.AppointmentCompleted.Amount_(€)')}</th>
                              <th rowspan="2">{t('Patient.AppointmentCompleted.Appointment_Type')}</th>
                              <th rowspan="2">{t('Patient.AppointmentCompleted.Diagnosis')}</th>
                              <th rowspan="2">{t('Patient.AppointmentCompleted.Feedback')}</th>
                            </tr>
                            <tr>
                          ...
                          {this.state && this.state.LoadedData &&
                            <tbody>
                                ...
                                <tr>
                                  ...
                                  <td className="admin-switch2">
                                    <Switch
                                      ...
                                      onChange={this.UpdateShowDiagnosisToOthers}
                                      offColor="#bdc1c2"
                                      onColor="#20CAD6"
                                      handleDiameter={15}
                                      width={30}
                                      height={15}
                                    />
                                    <Link to={`/patient-view-diagnostic/${v.appointmentGuid}`} className="doctorName">{t('Patient.AppointmentCompleted.View')}</Link>
                                  </td>
                                      ...
                                      <div className="dropdown-menu action-dropdown" aria-labelledby="dropdownMenuButton">
                                        {
                                          ...
                                          <a className="dropdown-item" onClick={this.ShowRefundPopUp.bind(this, v.appointmentGuid)}>Ask for refund <span className="dropdown-item-timer">{parseInt(v.refundPendingHours)} {t('Patient.AppointmentCompleted.Hours_Pending')}</span></a>
                                        }
                                        {v.isPaidOnline && v.isApplyForRefund === false && v.refundPendingHours === 0 &&
                                          <a className="dropdown-item">{t("Patient.AppointmentCompleted.Refund_time_left")}</a>
                                        }
                                        {v.isPaidOnline && v.isApplyForRefund === true && v.appointmentStatus === "Completed" &&
                                          <a className="dropdown-item">{t("Patient.AppointmentCompleted.Applyed_for_Refund")}</a>
                                        }
                                        {v.isPaidOnline && v.isApplyForRefund === true && v.appointmentStatus === "Refunded" &&
                                          <a className="dropdown-item"> {v.refundAmount} (€) {t("Patient.AppointmentCompleted.Refunded")} </a>
                                        }
                                        ...
                                        <Link className="dropdown-item" to={`/book-an-appoinment-doc-detail/${v.doctorGuid}`}>{t('Patient.ViewDignosticReport.Book_Again')}</Link>
                                        <a className="dropdown-item">{t("Patient.AppointmentCompleted.Invoice_receipt")}</a>
                                      </div>
                                    </div>
                                  </td>
                                  {/* <td>
                              ...
                              )) : <tr><td colSpan={8} className="empty-list">{t("EmptyListMessages.completed_appointments")}</td></tr>
                              }
                            </tbody>
                          }
                        </table>
            ...
            {this.state.viewAppointmentFeedBackPopup ? <AppointmentFeedBack CloseAppointmentFeedBack={this.toggleAppointmentFeedBackPopUp} AppointmentData={this.state.AppointmentData} /> : null}
            {this.state.viewRefundPopup ? <AskRefundPopup CloseAskRefundPopup={this.ShowRefundPopUp} AppoinmentDetails={this.state.RefundAppointmentData} /> : null}
            {this.state.FeedbackViewPopup === true && this.state.LoadedFeedBackData === true &&
                      ...
                      <div className="divForm my-4">
                        <div className="d-flex flex-column text-center">
                          <div className="patient-forget-password-text">
                            <p>{t("Patient.AppointmentCompleted.Patient_Feedback")}</p>
                          </div>
                        </div>
                        <label className="mb-2">{this.state.FeedbackData.patientFirstname} {this.state.FeedbackData.patientLastName}</label>
                        <div className="search-bar-text-input delete-account-textarea">
                          <label className="form-control">{this.state.FeedbackData.description}</label>
                        </div>
                        <div className="mt-2 mb-5 d-flex align-items-center">
                          <span className="feedback-star-rating-text">{t("Patient.AppointmentCompleted.Star_Rating")}</span>
                          <ReactStars
                        ...
                        <div className="mt-3 text-center">
                          <button type="button" className="btn pending-feedback-cancel w-25 mx-2" onClick={this.FeedbackViewPopUp.bind(this, "")}>{t("Patient.AppointmentCompleted.Ok")}</button>
                        </div>
                      </div>
                    </div>
                  </div>
...
export default connect(mapStoreToprops, mapDispatchToProps)(withTranslation()(PatientAppointmentCompleted));
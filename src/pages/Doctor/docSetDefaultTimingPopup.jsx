...
import { promiseWrapper, weekArray, stringToBoolean } from '../../utility/common';
...
class SetDefaultTiming extends React.Component {
  ...
  componentDidMount() {
    if (this.props.DefaultGUID !== "") {
      promiseWrapper(this.props.actions.getDoctorDefaultTimingById, { defaultGuid: this.props.DefaultGUID })
        .then((jsdata) => {
          let tempdata = jsdata.data;
          let tta = [];
          tta.push(tempdata.dayID);
          this.setState({ IsOnline: tempdata.isOnlineAppointment === true ? "True" : "False" });
  ...
  UpdateTiming = (field, e) => {
    let timeAndDate = moment(new Date().toLocaleDateString("en-US") + ' ' + e.target.value).format();
    if (field === "1") {
  ...
  saveDefaultTiming() {
    ...
    if (this.state.WorkFromTime !== '' && this.state.WorkToTime !== '' && this.state.WorkFromTime !== null && this.state.WorkToTime !== null) {
    ...
    // }
    if (this.state.LunchFromTime !== '' && this.state.LunchToTime !== '' && this.state.LunchFromTime !== null && this.state.LunchToTime !== null) {
    ...
    if (this.state.WorkFromTime) {
      if (this.state.WorkFromTime.indexOf('+') >= 0)
        sWorkFromTime = this.state.WorkFromTime.substring(0, this.state.WorkFromTime.indexOf('+'));
    ...
    if (this.state.WorkToTime) {
      if (this.state.WorkToTime.indexOf('+') >= 0)
        sWorkToTime = this.state.WorkToTime.substring(0, this.state.WorkToTime.indexOf('+'));
    ...
    if (this.state.LunchFromTime) {
      if (this.state.LunchFromTime.indexOf('+') >= 0)
        sLunchFromTime = this.state.LunchFromTime.substring(0, this.state.LunchFromTime.indexOf('+'));
    ...
    if (this.state.LunchToTime) {
      if (this.state.LunchToTime.indexOf('+') >= 0)
        sLunchToTime = this.state.LunchToTime.substring(0, this.state.LunchToTime.indexOf('+'));
    ...
    promiseWrapper(this.props.actions.saveDoctorDefaultTiming, { doctorDefaultTimingModel: response })
      .then((data) => {
  ...
  render() {
    ...
    return (
      <div className="modal" id="working-timing-modal" style={{ display: "block" }}>
        <div className="modal-dialog modal-dialog-centered doctor-my-timing-dialog">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title working-titming-modal-title" id="exampleModalLabel">{t('Doctor.Profile_Basic.Set_Working_Time')}</h5>
              <button type="button" onClick={this.handleSetDefaultTimingPopupClick} className="close close-btn" data-dismiss="modal" aria-label="Close">
            ...
            <div className="modal-body">
              <div className="row">
                <div className="col-md-6 pr-2">
                  <div className="search-bar-text-input mb-4">
                    <span>{t('Doctor.Profile_Basic.Select_Appoitnment_Type')}</span>
                    <div style={{ width: "100%" }} className="d-flex justify-content-between divForm-radio mt-1">
                ...
                <div className="col-md-6 pr-2">
                  <div className="form-group">
                    <div className="search-bar-text-input mb-4">
                      <label className="form-label">{t('Doctor.Profile_Basic.Duration_of_Patient_Consultation_(Minutes)')}</label>
                      <select value={this.state.TimeSlot}
                        onChange={this.UpdateTimeSlot.bind(this)}
                        className="physician-servicies-select form-control appearance-auto">
                        <option value="0">{t('Doctor.Profile_Basic.Select_Session_Duration')}</option>
                  ...
                  <div className="form-group">
                    {this.state.LoadedData === true &&
                      <div className="search-bar-text-input">
                        <label className="form-label">{t('Doctor.Profile_Basic.Select_Day')}</label>
                        <DropdownMultiselect
              ...
              <div className="row">
                <div className="col-md-6 pr-2">
                  <div className="search-bar-text-input mb-4">
                    <label className="form-label">{t('Doctor.Profile_Basic.Working_Time_From')}</label>
                    <input
                      type="time"
                ...
                <div className="col-md-6 pr-2">
                  <div className="search-bar-text-input mb-4">
                    <label className="form-label">{t('Doctor.Profile_Basic.Working_Time_To')}</label>
                    <input type="time" value={moment(this.state.WorkToTime).format('HH:mm')} onChange={this.UpdateTiming.bind(this, "2")} className="form-control" placeholder={t('Doctor.Profile_Basic.Select_Day')} />
                  </div>
                </div>
              </div>
              <div className="row">
                <div className="col-md-6 pr-2">
                  <div className="search-bar-text-input mb-4">
                    <label className="form-label">{t('Doctor.Profile_Basic.Lunch_Break_From_(optional)')}</label>
                    <input type="time" value={moment(this.state.LunchFromTime).format('HH:mm')} onChange={this.UpdateTiming.bind(this, "3")} className="form-control" placeholder={t('Doctor.Profile_Basic.Select_Day')} />
                  </div>
                </div>
                <div className="col-md-6 pr-2">
                  <div className="search-bar-text-input mb-4">
                    <label className="form-label">{t('Doctor.Profile_Basic.Lunch_Break_To_(optional)')}</label>
                    <input type="time" value={moment(this.state.LunchToTime).format('HH:mm')} onChange={this.UpdateTiming.bind(this, "4")} className="form-control" placeholder={t('Doctor.Profile_Basic.Select_Day')} />
                  </div>
                </div>
              </div>
            </div>
            <div className="modal-footer">
              <button type="button" className="btn btn-outline-secondary px-5 doctor-timing-modal-cancel-btn" onClick={this.handleSetDefaultTimingPopupClick} data-dismiss="modal">{t('Doctor.Profile_Basic.Cancel')}</button>
              <button type="button" onClick={this.saveDefaultTiming.bind(this)} className="btn px-5 doctor-timing-modal-save-btn">{t('Doctor.Profile_Basic.Save')}</button>
            </div>
          </div>
        </div>
      </div>
...
export default connect(mapStoreToprops, mapDispatchToProps)(withTranslation()(SetDefaultTiming));
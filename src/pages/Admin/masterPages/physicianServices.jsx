...
class PhysicianServices extends Component {
   ...
   getPhysicianServices = () => promiseWrapper(this.props.adminactions.getAllPhysicianServiceDetail)
      .then((data) => this.setState({ physicianServices: data.data }));
   ...
   savePhysicianService = (service = null, id = null) => {
      if (this.state.newService || id) {
         ...
         promiseWrapper(this.props.adminactions.savePhysicianService, { data: data })
            .then((data) => {
                  ...
                  <div className="row">
                     ...
                     <div className="col-md-12 col-sm-12 col-lg-10 mainRightPanel">
                        ...
                        <div className='appointment-details-container'>
                           <form className="appointment-details-form" onSubmit={(e) => { e.preventDefault(); this.savePhysicianService() }}>
                              <div className="row">
                        ...
                        <div className="row">
                           {physicianServices.length > 0 ?
                              physicianServices.map((service, i) =>
                                 ...
                                 >
                                    <div className="d-flex justify-content-between btn doctor-report-edit-btn"
                                       style={{ backgroundColor: "#0dcaf0", color: "white" }}>
                                       <EdiText
                                          ...
                                          editButtonContent={<i className="fas fa-pen" style={{ color: "white" }}></i>}
                                          showButtonsOnHover={true}
                                          cancelOnUnfocus={true}
                                          submitOnEnter={true}
                                          onSave={(name) => this.savePhysicianService(name, service.physicianServiceId)}
                                       />
                                    </div>
                                 </div>
                              ) : <div className="col-lg-3 my-2 px-2 empty-list">{t("EmptyListMessages.physician_list")}</div>
                           }
                        </div>
...
export default connect(mapStoreToprops, mapDispatchToProps)(withTranslation()(PhysicianServices));
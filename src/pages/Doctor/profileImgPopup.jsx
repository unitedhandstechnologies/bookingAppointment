...
class ProfileImgPopup extends React.Component {
    ...
    handleSave = (data) => {
        ...
        let modelData = {
            userGuid: localStorage.getItem("user-id"), imgData: img
        }

        promiseWrapper(this.props.actions.uploadProfileImage, { model: modelData })
            .then((jsdata) => {
    ...
    render() {
            ...
            <div className="modal" style={{ display: "block" }}>
                <div className="modal_content p-0 top-10">
                    <div className="modal-header">
                        <h5 className="modal-title">{t('Doctor.ProfileImgPopup.Upload_Image')}</h5>
                        <span className="close h4" onClick={this.handlePopupClick}>&times;</span>
                    </div>
                    <div className="modal-body d-flex flex-column align-items-center">
                                    ...
                                    <div className="search-bar-text-input">
                                        <div>
                                            <label className="btn fileUpload btn-default">
                                                {t('Doctor.ProfileImgPopup.Upload_Image')} <input
                                                    name="newImage"
                                                    type="file"
                        ...
                        <div className="d-flex align-items-center mt-3">{t('Doctor.ProfileImgPopup.Zoom:')}&nbsp;
                            <input
                                className="mt-1"
                                name="scale"
                                type="range"
                    ...
                    />
                    <br /> */}
                        <div className="d-flex align-items-center mt-3">{t('Doctor.ProfileImgPopup.Border_radius:')}&nbsp;
                            <input
                                className="mt-1"
                                name="scale"
                                type="range"
        ...
        Y Position:
                <input
                    ...
                    />
                    <br /> */}
                        <div className="d-flex align-items-center mt-3 w-100 justify-content-center">{t('Doctor.ProfileImgPopup.Rotate:')}:&nbsp;
                            <button onClick={this.rotateLeft} className="btn fileUpload btn-default px-4 mx-1">{t('Doctor.ProfileImgPopup.Left')}</button>
                            <button onClick={this.rotateRight} className="btn fileUpload btn-default px-4">{t('Doctor.ProfileImgPopup.Right')}</button>
                        </div>
        ...
        Transparent image?
                <input type="checkbox" onChange={this.setTransparent} defaultChecked={this.state.isTransparent}></input>
                    ...
                    <div className="modal-footer">
                        ...
                        <button type="button" className="btn btn-primary" onClick={this.handleSave}>{t('Doctor.ProfileImgPopup.Save_changes')}</button>
                        <button type="button" className="btn btn-secondary" onClick={this.handlePopupClick} data-dismiss="modal">{t('Doctor.ProfileImgPopup.Close')}</button>
                    </div>
...
export default connect(mapStoreToprops, mapDispatchToProps)(withTranslation()(ProfileImgPopup));
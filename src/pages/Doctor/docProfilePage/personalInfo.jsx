...
export class PersonalInfo extends Component {
  ...
  selectDefaultlanguageId = (languageId) => {
    if (languageId && this.props.WebSiteLanguageData.length) {
      const defLanguageOption = this.props.WebSiteLanguageData.find(language => languageId === language.languageId);
  ...
  selectDefaultTimezone = (timezoneGuid) => {
    if (timezoneGuid && this.props.TimezoneData.length) {
      const defTimezoneOption = this.props.TimezoneData.find(timezone => timezoneGuid === timezone.timezoneGuid);
  ...
  selectDefaultcitizenshipId = (citizenshipId) => {
    if (citizenshipId && this.props.CitizenshipData.length) {
      const defCitizenshipOption = this.props.CitizenshipData.find(citizenship => citizenshipId === citizenship.countryID);
      return { label: defCitizenshipOption.citizenship, value: defCitizenshipOption.countryID }
    }
  }

  render() {
    const { t, WebSiteLanguageData, CitizenshipData, TimezoneData, LanguageData, onSubmit } = this.props;
    const { ProfilePersonalInfo } = this.state
    return (
      <Formik
        enableReinitialize
        initialValues={{
      ...
      >{({ handleSubmit, errors, touched, handleChange, values }) => (
        <Form className="divForm" onSubmit={(e) => { e.preventDefault(); handleSubmit(e) }}>
          <div className="row pt-2">
            <div className="col-md-6 mt-4">
              <div className="search-bar-text-input mb-4">
                ...
                <Select
                  name='languageId'
                  className={`${errors.languageId && touched.languageId && "error-message-input"}`}
                  onChange={selectedOption => {
                    let event = { target: { name: 'languageId', value: selectedOption.value } }
                    handleChange(event)
                  }}
            ...
            <div className="col-md-6">
              <div className="form-group">
                <div className="search-bar-text-input mb-4">
                  <label className="form-label">{t('Doctor.Profile_Basic.Supported_Languages')}</label>
                  <DropdownMultiselect
                    ...
                    placeholder={t('Doctor.Profile_Basic.Select_Preferred_Language')}
                    handleOnChange={selectedOption => {
                      let event = { target: { name: 'preferredLanguageIds', value: selectedOption } }
                      handleChange(event)
                    }}
          ...
          <div className="row">
            <div className="col-md-6">
              <div className="search-bar-text-input mb-4">
                <label className="form-label">{t('Doctor.Profile_Basic.First_Name')}</label>
                <Field type="text"
                  className="form-control"
                  placeholder={t('Doctor.Profile_Basic.First_Name')}
                  name='firstName'
                  onChange={selectedOption => {
                    let event = { target: { name: 'firstName', value: selectedOption.value } }
                    handleChange(event)
                  }}
                />
              </div>
          ...
          <div className="row">
            <div className="col-md-6">
              <div className="search-bar-text-input mb-4">
                <label className="form-label">{t('Doctor.Profile_Basic.Timezone')}</label>
                <Select
                  name='timezoneGuid'
          ...
          <div className="w-100">
            <div className="mb-5 d-flex justify-content-end">
              <input type="submit"
                className="btn MyButton register-button w-25" value={t('Doctor.Profile_Basic.Save_Next')} />
            </div>
          </div>
        </Form>
      )}

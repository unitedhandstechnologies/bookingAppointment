...
class CmsFAQs extends Component {
   constructor(props) {
      super(props);
      this.state = {
         isAdmin: true,
   ...
   componentDidMount() {
      this.getCMSPageData(cmsPageNum.FAQs, this.props.langId ? this.props.langId : 1, this.state.isAdmin)
   }
   ...
   componentDidUpdate() {
      if (this.props.langId !== this.state.langId) {
         this.setState({ langId: this.props.langId, cmsData: { ...this.state.cmsData, languageId: this.props.langId } }, () => {
            this.getCMSPageData(cmsPageNum.FAQs, this.props.langId, this.state.isAdmin)
         })
      }
   }
...
export default connect(mapStoreToProps, mapDispatchToProps)(CmsFAQs);
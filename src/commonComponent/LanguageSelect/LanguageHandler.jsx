...
import { COOKIES_KEY, localStorageKeys, promiseWrapper } from '../../utility/common';
        ...
        return (
            <li className="nav-item dropdown position-static d-flex align-items-center">
                {this.state && this.state.LoadedData &&
                    <div>
                        <div className="nav-link dropdown-toggle" id="navbarDropdown" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">
                            {languageName}
                        </div>
                        <div className="dropdown-menu dropdown-menu-first" aria-labelledby="navbarDropdown">
                            <div className="row">
                                {this.state.WebSiteLanguageData.map((languageData, idx) => (
                                    <div className="col-md-6" key={idx}>
                                        <button className="dropdown-item" onClick={() => this.changeStateLanguage(languageData)}>{languageData.languageName}</button>
                                    </div>
                                ))}
                            </div>
                        </div>
...
export default connect(mapStoreToprops, mapDispatchToProps)(withTranslation()(LanguageHandler));
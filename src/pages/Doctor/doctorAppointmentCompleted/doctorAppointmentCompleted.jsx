...
class DoctorAppointmentCompleted extends React.Component {
  ...
  setGenerateInvoiceModal = (value) => this.setState({ generateInvoiceModal: value })
                  ...
                  <div className="row my-4">
                    <div className="search-bar-text-input col-md-7 top-search">
                      ...
                      <div className="col-lg-4 float-start">
                        <input type="text"
                          className="form-control"
                          placeholder={t('Doctor.AppointmentCompleted.Search_Appointment_Doctor_Name')}
                    ...
                    <div className="search-bar-text-input col-md-2">
                      <button type="button" onClick={this.GetAppointmentRequestList.bind(this)} className="btn filter-btn w-100">{t('Doctor.AppointmentCompleted.Search')}</button>
                    </div>
                  </div>
                        ...
                        <table className="table table-bordered appointmentTable">
                          <thead>
                            <tr className="new-patient-table-title">
                              <th rowSpan="2">{t('Doctor.AppointmentCompleted.Booking_ID')}</th>
                              <th rowSpan="2">{t('Doctor.AppointmentCompleted.Patient_Name')}</th>
                              <th>{t('Doctor.AppointmentCompleted.Date')}</th>
                              <th rowSpan="2">{t('Doctor.AppointmentCompleted.Medical_Specialty')}</th>
                              <th rowSpan="2">{t('Doctor.AppointmentCompleted.Amount_(€)')}</th>
                              <th rowSpan="2">{t('Doctor.AppointmentCompleted.Appointment_Type')}</th>
                              <th rowSpan="2">{t('Doctor.AppointmentCompleted.Diagnosis')}</th>
                              <th rowSpan="2">{t('Doctor.AppointmentCompleted.Feedback')}</th>
                              <th rowSpan="2">{t("Doctor.AppointmentCompleted.Action")}</th>
                            </tr>
                            <tr>
                            ...
                            <tbody>
                              {this.state.AppointmentRequestList.length > 0 ? this.state.AppointmentRequestList.map((v, idx) => (
                                <tr key={idx}>
                                  ...
                                  <td><Link to={`/doc-view-diagnostic/${v.appointmentGuid}`} className="doctorName">{t('Doctor.AppointmentCompleted.View')}</Link></td>
                                  <td>
                                    {v.isFeedbackGiven === true &&
                                      <a onClick={this.FeedbackViewPopUp.bind(this, v.appointmentGuid)} className="type">{t('Doctor.AppointmentCompleted.Given')}</a>
                                    }
                                    {v.isFeedbackGiven === false && v.feedbackPendingHours > 0 &&
                                      <span className="status">{t('Doctor.AppointmentCompleted.Pending')}</span>
                                    }
                                    {v.isFeedbackGiven === false && v.feedbackPendingHours === 0 &&
                                      <span className="country">{t('Doctor.AppointmentCompleted.Expired')}</span>
                                    }
                                  </td>
                                    ...
                                    <div className="dropdown">
                                      ...
                                      <div className="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                        <button className="dropdown-item"
                                          onClick={() => { this.setState({ invoiceGuid: v.appointmentGuid }); this.setGenerateInvoiceModal(true) }}
                                        >
                                          {t("Doctor.AppointmentCompleted.Generate_Invoice")}
                                        </button>
                                        <button className="dropdown-item"
                                        >
                                          {t("Doctor.AppointmentCompleted.View_Invoice")}
                                        </button>
                                      </div>
                                    </div>
                                  </td>
                                </tr>
                              )) : <tr><td colSpan={9} className="empty-list">{t("EmptyListMessages.completed_appointments")}</td></tr>
                              }
                            </tbody>
                          }
                        </table>
                      ...
                      <div className="divForm my-4">
                        <div className="d-flex flex-column text-center">
                          <div className="patient-forget-password-text">
                            <p>{t('Doctor.AppointmentCompleted.Patient_Feedback')}</p>
                          </div>
                        </div>
                        ...
                        <div className="mt-2 mb-5 d-flex align-items-center">
                          <span className="feedback-star-rating-text">{t('Doctor.AppointmentCompleted.Star_Rating')}</span>
                          <ReactStars
                        ...
                        <div className="mt-3 text-center">
                          <button type="button" className="btn pending-feedback-cancel w-25 mx-2" onClick={this.FeedbackViewPopUp.bind(this, "")}>{t('Doctor.AppointmentCompleted.Ok')}</button>
                        </div>
                      </div>
                    </div>
                  </div>
...
export default connect(mapStoreToprops, mapDispatchToProps)(withTranslation()(DoctorAppointmentCompleted));
...
class CmsTandCDoc extends Component {
   constructor(props) {
      super(props);
      this.state = {
         isAdmin: true,
   ...
   componentDidUpdate() {
      if (this.props.langId !== this.state.langId) {
         this.setState({
            ...
            this.getCMSPageData(cmsPageNum.TermsNConditionDoctor, this.props.langId, this.state.isAdmin)
         })
      }
   }
   ...
   getCMSPageData = (pageNum, langId, isAdmin) => {
      ...
      promiseWrapper(this.props.comactions.getCMSPage, { query: { ...queryParam } })
         .then(data => {
            const receivedData = {
               ...data.data,
               pageNumber: cmsPageNum.TermsNConditionDoctor,
...
export default connect(mapStoreToProps, mapDispatchToProps)(CmsTandCDoc);